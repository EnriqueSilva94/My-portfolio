//3. create an array called buttonColours
...
function startOver () {
    ...
    started = false;
    level = 0;
    $("#level-title").text("Press A key to Stat");
...
function nextSecuence() {
    ...
    //4.  create a variable called randomChosenColour and use the randomNumber from step 2 to select a random colour from the buttonColours array.
    var randomChosenColour = buttonColours[randomNumber];

    //6. Add the new randomChosenColour generated in step 4 to the end of the gamePattern.
    gamePattern.push(randomChosenColour);

    //1-2 Use jQuery to select the button with the same id as the randomChosenColour/
...
function animatePress(currentColour) {
    ...
    //6.4. Use jQuery to add this pressed class to the button that gets clicked inside animatePress().
    $("#" + currentColour).addClass("pressed");

    //6.5. use Google/Stackoverflow to figure out how you can use Javascript to remove the pressed class after a 100 milliseconds.
    setTimeout(function(){    
